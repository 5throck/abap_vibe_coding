...
async function main(): Promise<void> {
  console.log("?îç Verifying Skills\n");
  ...
  for (const check of checks) {
    const icon = check.status === "PASS" ? "?? : check.status === "WARN" ? "?†Ô∏è" : "??;
  ...
  if (failed > 0) {
    ...
    console.log(`?†Ô∏è  ${warned} warnings`);
  } else {
...
async function scanSkills(): Promise<SkillCheck[]> {
  ...
  async function scanDirectory(dir: string): Promise<string[]> {
    ...
    if (!existsSync(skillsPath)) return files;

    const globber = new Bun.Glob("**/*");
    for await (const entry of globber.scan({ cwd: skillsPath, onlyFiles: true })) {
...
async function generateSkillsIndex(checks: SkillCheck[]): Promise<void> {
  ...
  console.log(`\n?ìù Generated skills index: ${indexPath}`);
}
