...
> **Hook limitation**: `PostToolUse` hooks configured in `.claude/settings.json` do **not** fire in the Desktop App. After any `WriteSource` / `EditSource`, run the Post-Write Mandatory Chain manually (see `SKILL.md § Post-Write Mandatory Chain`) and sync via `scripts/vsp-sync.sh` or `scripts/vsp-sync.ps1`.
